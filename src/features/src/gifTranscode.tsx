...
export type GIFConfiguration = {
  'TRANSCODE': { 'FORMAT': { 'CONFIG': { fps: number, loop: number, scale: number }, value: 'GIF', [name: string]: any } },
  [name: string]: any
}
...
class GIFTranscode extends FFmpegFeature {
  ...
  parseConfiguration = () => {
    ...
    const { CONFIG } = FORMAT;
    console.info('GIF CONFIG', JSON.stringify(CONFIG), 'GIF FORMAT', JSON.stringify(FORMAT), 'Direct', JSON.stringify(configuration.TRANSCODE.FORMAT));
    ...
    <>
      <div className="flex flex-wrap -mx-3 mb-2">
        <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
          <label className="block uppercase tracking-wide text-white text-xs font-bold mb-2" htmlFor="fps">
            fps (Frames per second)
          </label>
          <input ref={fpsRef} className="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="fps" type="number" placeholder="10" />
        </div>
        <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
          <label className="block uppercase tracking-wide text-white text-xs font-bold mb-2" htmlFor="scale">
            Width
          </label>
          <input ref={scaleRef} className="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="scale" type="number" placeholder="320" />
        </div>
      </div>
      <div className="flex flex-wrap -mx-3 mb-2">
        <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
          <label className="block uppercase tracking-wide text-white text-xs font-bold mb-2" htmlFor="loop">
            Loops (0 - Infinite, -1  No Loop)
          </label>
          <input ref={loopRef} className="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="loop" type="number" placeholder="0" />
        </div>
      </div>
    </>
  );
