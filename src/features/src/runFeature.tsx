...
class RunFeature extends FFmpegFeature {
  ...
  setFFmpegCommands(command: string) {
    if (command.includes('-i') || command.includes('-o')) {
      throw new Error(
        'Invalid Custom Command, please do not use -i or -o to input or output any files'
      )
    }
    ...
    <div className="flex flex-col">
      <div className="flex flex-wrap -mx-3 mb-6">
        <div className="w-full px-3">
          <p className="text-l font-bold w-full text-white pb-6">
            This is likely to not work unless you know what you are doing
          </p>
          <label
            className="block uppercase tracking-wide text-m font-bold mb-2"
            htmlFor="run-cmd">
            Custom Run Command
          </label>
          <textarea
          ...
          <p className="text-red-500 text-s italic">
            Please do not add any input or output files here. No -i or -o
          </p>
          <p className="text-gray-600 text-s italic">
            Currently only support one video input file
          </p>
        </div>
      </div>
      <div className="flex flex-wrap -mx-3 mb-6">
        <div className="w-full px-3">
          ...
          <input
            className="appearance-none block w-full bg-white border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none  focus:border-gray-500"
            id="run-out"
            type="text"
