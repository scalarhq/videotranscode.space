...
const TerminalComponent: React.FC<TerminalProps> = ({ terminalText, clearTerminal, loaded, updateTerminalText, updateClearTerminal }) => {
	const [t1, updateT1] = useState(new Terminal("terminal"))
	...
	useEffect(() => {
		...
		// updateT1(new Terminal("terminal"))
		...
		// i assume we only wanna do this once. yes
		updateT1(newTerminal(t1))
		// @ts-ignore
		window.t1 = t1;
		...
		let terminalDiv = terminalRef.current;
		if (terminalDiv !== null) {
			// @ts-ignore lol
	...
	return (<div id="terminal" ref={terminalRef} style={{ display: "flex", flex: "0 1", height: "100%" }} />
	)
}
