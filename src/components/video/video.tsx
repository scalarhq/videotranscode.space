...
const GifDisplay = ({ url }: { url: string }) => {
  ...
  return (
    <div style={{ width: '40vw' }}>
      <img src={url} style={{ width: '100%', height: 'auto', objectFit: 'cover' }} />
    </div>
  )
}
...
const DownloadButton = ({ url, ext }: { url: string, ext: string }) => {
  ...
  return (
    <div className="row" style={{ paddingTop: '3%' }}>
      ...
      <a id="download" ref={downloadButton} data-testid="download-button" class="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 rounded inline-flex items-center">
...
const VideoWrapper = ({ url, toDisplay, ext }: VideoProps) => {
  ...
  return (
    <div>
      {toDisplay ? <Video url={url} /> : ext === "gif" ? <GifDisplay url={url} /> : null}
