 ...
 */
export const getFFmpegFlags = (codecType = formatType.ffmpegLib, compressionLevel) => {
    if (compressionLevel < codecType.compressionRange.min && 
        compressionLevel > codecType.compressionRange.max) 
            throw new Error(`Compression level is not within codec range: ${CODEC_TYPES.name}`)
}
